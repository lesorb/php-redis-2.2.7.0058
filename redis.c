...
/*
  ...
  | Maintainer: Lesorb <lesorb@gmail.com>                                |
  +----------------------------------------------------------------------+
*/
...
#define LES_MCRC_NUM_SLTOS 16384
...
static zend_function_entry redis_functions[] = {
     ...
     /* To obtain a sequence of master server by the key */
     PHP_ME(Redis, getMasterByKey, NULL, ZEND_ACC_PUBLIC)
...
/* {{{ proto string Redis::getMasterByKey(string key, string _master_servers)
 */
PHP_METHOD(Redis, getMasterByKey)
{
    zval *object, *master_servers;
    char *key = NULL;
    HashTable *hash;
    int key_len, cmd_len;
    int key_free, slot;
    int arg_count, address;

    if (zend_parse_method_parameters(ZEND_NUM_ARGS() TSRMLS_CC, getThis(), "Osa",
                                     &object, redis_ce,
                                     &key, &key_len, &master_servers) == FAILURE) {
        RETURN_FALSE;
    }

    /* Grab our array */
    hash = Z_ARRVAL_P(master_servers);
    ...
    slot = les_crc16(key) % LES_MCRC_NUM_SLTOS;
    address = floor(slot / (int)(LES_MCRC_NUM_SLTOS / arg_count));
